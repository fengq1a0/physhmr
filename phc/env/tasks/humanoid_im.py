...
class HumanoidIm(humanoid_amp_task.HumanoidAMPTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        if self.kin_loss:
            ...
            self.ori_obs_buf = torch.zeros((self.num_envs, 934), device=self.device, dtype=torch.float)
        ...
        if self.humanoid_type in ["smpl", "smplh", "smplx"]:
            ...
            if not flags.test:
                ...
                self._motion_amp_lib.load_motions(skeleton_trees=self.skeleton_trees, gender_betas=self.humanoid_shapes.cpu(),
                                              limb_weights=self.humanoid_limb_and_weights.cpu(), random_sample=not self.seq_motions,
                                              max_len=self.max_len, start_idx=self.start_idx)
    ...
    def post_physics_step(self):
        ...
        if flags.im_eval:
            ...
            pose_quat = (sRot.from_quat(body_quat.reshape(-1, 4).numpy()) * self.pre_rot).as_quat().reshape(-1, 24, 4)
            new_sk_state = SkeletonState.from_rotation_and_root_translation(self.skeleton_trees[0], torch.from_numpy(pose_quat), body_pos[:,0,:].cpu(), is_local=False)
            local_rot = new_sk_state.local_rotation
            pose_aa = sRot.from_quat(local_rot.reshape(-1, 4).numpy()).as_rotvec().reshape(-1, 24, 3)



            tmp_rot = (sRot.from_quat(motion_res["rb_rot"].reshape(-1, 4).numpy()) * self.pre_rot).as_quat().reshape(-1, 24, 4)
            new_sk_state = SkeletonState.from_rotation_and_root_translation(self.skeleton_trees[0], torch.from_numpy(tmp_rot), body_pos[:,0,:].cpu(), is_local=False)
            local_rot = new_sk_state.local_rotation
            tmp_rot = sRot.from_quat(local_rot.reshape(-1, 4).numpy()).as_rotvec().reshape(-1, 24, 3)
            ...
            self.extras['body_pos'] = body_pos.cpu().numpy().copy()#[:,self.mujoco_2_smpl, :]
            self.extras["body_rot"] = pose_aa.copy()
            self.extras['body_pos_gt'] = motion_res['rg_pos'].cpu().numpy().copy()
            self.extras["body_rot_gt"] = tmp_rot.copy()
        ...
        if self.obs_v == 1 :
            ...
            if self.obs_v == 4 or self.obs_v == 6:
                ...
                obs = compute_imitation_observations_v66(
                    motion_res["visual_feat"][:,1:512+1],
                ...
                obs = motion_res["visual_feat"]
            elif self.obs_v == 358: # kp2d + rotation
                obs = compute_imitation_observations_v358(
                    motion_res["visual_feat"],
                    root_pos, root_rot, body_pos_subset, body_rot_subset, body_vel_subset, 
                    body_ang_vel_subset, ref_rb_pos_subset, ref_rb_rot_subset, ref_body_vel_subset, 
                    ref_body_ang_vel_subset, time_steps, self._has_upright_start)
            elif self.obs_v == 359: # kp2d only
    ...
    def _compute_reset(self):
        ...
        if self.zero_out_far and self.zero_out_far_train:
            ...
            # Disable early termination during testing; metrics are computed from exported results
            enable_et = self._enable_early_termination and not flags.test
            self.reset_buf[:], self._terminate_buf[:] = compute_humanoid_im_reset(  # Humanoid reset
                self.reset_buf, self.progress_buf, self._contact_forces, self._contact_body_ids, self._rigid_body_pos[..., self._reset_bodies_id, :], ref_rb_pos[..., self._reset_bodies_id, :], pass_time, enable_et, self._termination_distances[..., self._reset_bodies_id],
            ...
            self.reset_buf[:], self._terminate_buf[:] = compute_humanoid_im_reset(self.reset_buf, self.progress_buf, self._contact_forces, self._contact_body_ids, \
                                                                               body_pos, ref_body_pos, pass_time, enable_et,
...
def compute_imitation_observations_v66(feature, root_pos, root_rot, body_pos, body_rot, body_vel, body_ang_vel, ref_body_pos, ref_body_rot, ref_body_vel, ref_body_ang_vel, time_steps, upright):
    ...
    ##### body pos + Dof_pos This part will have proper futuers.
    #root_pos += torch.randn_like(root_pos)*0.1
    local_ref_body_pos = ref_body_pos.view(B, time_steps, J, 3) - root_pos.view(B, 1, 1, 3)  # preserves the body position
    local_ref_body_pos = torch_utils.my_quat_rotate(heading_inv_rot_expand.view(-1, 4), local_ref_body_pos.view(-1, 3))
    local_ref_body_pos = local_ref_body_pos.view(B, time_steps, J, 3)[:,:,0:1,:]

#    local_ref_body_pos[:,:,:,1] +=1
#    local_ref_body_pos += torch.randn_like(local_ref_body_pos) * 0.5
...
def save_colored_point_cloud_with_lines(src, tar, cam, path):
    """
    src: [N, 3]
    tar: [N, 3]
    cam: [1, 3]
    path: output .ply path prefix
    """
    N = src.shape[0]
    
    # Combine points: [src, tar, cam]
    points = np.concatenate([src, tar, cam], axis=0)  # [2N+1, 3]

    # Point colors
    src_color = [1.0, 0.0, 0.0]   # red
    tar_color = [0.0, 0.8, 0.0]   # green
    ...
    bright_color = [1.0, 1.0, 0.0]  # bright yellow for src-tar
    dull_color = [0.5, 0.5, 0.5]    # gray for cam-tar

    for i in range(N):
        lines.append([i, i + N])           # src ↔ tar
        line_colors.append(bright_color)

        lines.append([2 * N, i + N])       # cam ↔ tar
        line_colors.append(dull_color)
    ...
    # Save to disk
    o3d.io.write_point_cloud(path.replace(".ply", "_points.ply"), pcd)
    o3d.io.write_line_set(path.replace(".ply", "_lines.ply"), line_set)
...
def compute_imitation_observations_v358(feature, root_pos, root_rot, body_pos, body_rot, body_vel, body_ang_vel, ref_body_pos, ref_body_rot, ref_body_vel, ref_body_ang_vel, time_steps, upright):
    ...
    heading_inv_rot_kp2d = heading_inv_rot.unsqueeze(-2).repeat((1, 12, 1)).repeat_interleave(time_steps, 0)
    ...
    smpl_id = [16, 17, 18, 19, 20, 21, 1, 2, 4, 5, 7, 8]
    kp2d    = feature[:,516:].view(B, 12, 4)     # [B, 12, 4]
    weight  = kp2d[:,:,3:]                       # [B, 12, 1]
    tmp_pos = body_pos[:,m2smpl,:][:,smpl_id,:]  # [B, 12, 3]
    ...
    dist_vec = tmp_pos - proj_pos                # [B, 12, 3]

#    print(weight.shape, dist_vec.shape)

    dist_vec = torch_utils.my_quat_rotate(heading_inv_rot_kp2d.view(-1, 4), dist_vec.view(-1, 3)).view(B, 12, 3)
...
#    obs.append(local_ref_body_pos.view(B, time_steps, 3))
    ...
    obs.append(local_ref_body_rot.view(B, time_steps, 6))
    obs.append(dist_vec.view(B, time_steps, 36))
    obs.append(weight.view(B, time_steps, 12))
