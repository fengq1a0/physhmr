...
class MotionLibBase():
    ...
    def load_data(self, motion_file,  min_length=-1, im_eval = False):
        ...
        self.tmp_scale_list = []
        for tmp in self._motion_data_list:
            self.tmp_scale_list.append(tmp.get("scale", 1.0))
        self.tmp_scale_list = np.array(self.tmp_scale_list)
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1):
        ...
        self.curr_scale = self.tmp_scale_list[sample_idxes]
        ...
        for f in tqdm(range(len(res_acc))):
            motion_file_data, curr_motion, tmp_visual_feat = res_acc[f]
            if USE_CACHE:
            ...
            visual_feat.append(tmp_visual_feat)
        ...
        self.visual_feat_device = torch.cat(visual_feat, dim=0).to(self._device)
    ...
    def get_motion_state(self, motion_ids, motion_times, offset=None):
        ...
        return {
            ...
            "visual_feat" : self.visual_feat_device[f0l]
        }
