...
class MotionLibSMPL(MotionLibBase):
    ...
    def fix_trans_height(pose_aa, trans, curr_gender_betas, mesh_parsers, fix_height_mode):
        ...
        with torch.no_grad():
            ...
            if fix_height_mode == FixHeightMode.ankle_fix:
                ...
                diff_fix = ((vertices_curr[:, pick] - offset[:, None])[:frame_check, ..., -1].min(dim=-1).values - height_tolerance).min()  # Only acount the first 30 frames, which usually is a calibration phase.
            elif fix_height_mode == FixHeightMode.full_fix:
                
                diff_fix = ((vertices_curr - offset[:, None])[:frame_check, ..., -1].min(dim=-1).values - height_tolerance).min()  # Only acount the first 30 frames, which usually is a calibration phase.
    ...
    def load_motion_with_skeleton(ids, motion_data_list, skeleton_trees, shape_params, mesh_parsers, config, queue, pid):
        ...
        for f in pbar:
            ...
            # Pack image features and auxiliary info into visual_feat
            visual_feat = []
            visual_feat.append(trans_fix.float()[None].expand(B, 1))
            for kk in ["img_feat", "camera", "kp2d"]: # , "kp2d", "bbox", "T_c2g", "poses_g"                             ## , "camera", "kp2d"
...
#                    if kk == "kp2d":
#                        tid = [0,1,2,3,4,5,8,9,10,11]
#                        tmp = tmp[:,tid,:]
                tmp = tmp.reshape(B, -1)
                visual_feat.append(tmp)
            visual_feat = torch.from_numpy(np.concatenate(visual_feat, axis=1)).float()
            ########################################
            res[curr_id] = (curr_file, curr_motion, visual_feat)
